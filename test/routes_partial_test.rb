...
class RoutesPartialTest < Minitest::Test
  ...
  def test_the_texts_are_valid_json_in_both_locales
    [EN, PT].each do |pagina|
      ...
      %w[play pause step speed restart clock queue arrival in_time apply reset remove instant kmh editor_hint unreachable legend_open legend_closed legend_planned legend_driven failed cancel changes now road invalid legend_edited edit keyboard_hint].each do |chave|
