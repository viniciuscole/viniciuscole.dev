...
have a speed, and the speed **changes while the car is driving**: a jam at
8:07am, a lane opening up at 8:20am. The program takes the map, the origin,
the destination, and the list of changes, and answers where to go and how
long it takes.

The simulation above runs the project's original C code, compiled to
WebAssembly. Hit **Run** to watch the algorithm pick a path, the car
follow it, and the detour once road 2→4 jams. Click any road to make your
own traffic jam.
...
destination is closed: it empties the whole queue. It could have stopped
there. That got left out.
...
O((V + E) log V). On a city map, that's the difference between seconds and
milliseconds.

Our heap has no "decrease-key". When a node's time improves, it's
inserted again, and the old copy stays in the queue with the stale time.
When it comes out, it's ignored because the node is already closed. Simpler
to write, and the cost is small: the queue grows to as many as E entries
instead of V.
...
This was the assignment. The car sets out on `1 2 4 6`, the fastest
plan: 3 km, 3 minutes. At 30 seconds, road 2→4 drops to 5 km/h. The car
only finds out on reaching 2, at 60 seconds. Then the program applies the
change and runs Dijkstra again, from 2. Road 2→4 now costs 12 minutes;
`2 5 6` costs 3. The car takes the detour and arrives in 4 minutes, having
driven 4 km.
...
chance to react. Of the whole model, this is the part that looks most like
an actual street.

<div class="rotas" data-rotas="via-libera"></div>

The opposite happens too. With everything at 30 km/h, the plan is
`1 2 4 6`, 6 minutes. At 60 seconds, road 2→5 opens up at 120 km/h. The
car reaches 2 at 2 minutes, replans, and `2 5 6` now costs 2 min 15
against 4 minutes through 4. It takes the detour and saves almost two
minutes.

## What I would do differently

- Recomputing from scratch on every update is simple and, for graphs this
  size, instant. On a real map I'd look at D\* Lite or LPA\*, which reuse
  the previous search and only fix what the change affected.
- The program reads the future. All the updates come from the file, each
  with its instant already marked. A real car only learns about a jam when
  it happens, but the Dijkstra it runs on every piece of news is exactly
  the one described here.
- Every road starts at the same speed. One more column in the file would
  be enough to give each road its own.
- The car never turns around. If the best move were to reverse on the road
  it's on, the model doesn't know it.
...
[repository](https://github.com/viniciuscole/car-routes-optimazing). I
added the *trace* mode that feeds these simulations for this page.
