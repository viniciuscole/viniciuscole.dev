...
engarrafamento às 8h07, pista liberada às 8h20. O programa recebe o mapa, a
origem, o destino e a lista de mudanças, e responde por onde ir e quanto
tempo leva.
...
a fila. Isso é o *relaxamento*. No código:
...
mais e as vias só somam tempo. Isso exige pesos positivos. Tempo sempre é.
...
esvazia a fila. Dava para parar ali. Ficou de fora.
...
e o algoritmo inteiro sairia em O(V²). Com o heap, O((V + E) log V). Num
mapa de cidade, é a diferença entre segundos e milissegundos.

O nosso heap não tem "diminuir chave". Quando o tempo de um nó melhora,
ele é inserido de novo, e a cópia antiga fica na fila com o tempo velho. Ao
sair, ela é ignorada porque o nó já foi fechado. Mais simples de escrever, e
o custo é pequeno: a fila cresce até E entradas em vez de V.

## Quando o trânsito muda

<div class="rotas" data-rotas="engarrafamento"></div>

Isto era o enunciado. O carro sai por `1 2 4 6`, o plano mais rápido:
3 km, 3 minutos. Aos 30 segundos a via 2→4 cai para 5 km/h. O carro só fica
sabendo ao chegar em 2, aos 60 segundos. Aí o programa aplica a mudança e
roda o Dijkstra de novo, a partir de 2. A via 2→4 passou a custar 12
minutos; `2 5 6` custa 3. O carro desvia e chega em 4 minutos, com 4 km
rodados.
...
mudou e o carro pagou por ele sem chance de reagir. De todo o modelo, é a
parte que mais parece com a rua.
...
- Recalcular do zero a cada atualização é simples e, para grafos deste
  tamanho, instantâneo. Num mapa real eu olharia D\* Lite ou LPA\*, que
  reaproveitam a busca anterior e só corrigem o que a mudança afetou.
- O programa lê o futuro. As atualizações vêm todas no arquivo, com instante
  marcado. Um carro de verdade só sabe do engarrafamento quando ele
  acontece, mas o Dijkstra que ele roda a cada notícia é exatamente este.
- Todas as vias começam na mesma velocidade. Bastaria uma coluna a mais no
  arquivo para cada uma ter a sua.
- O carro não dá meia-volta. Se a melhor saída fosse voltar pela via em que
  está, o modelo não sabe.
...
modo *trace* que alimenta estas simulações eu adicionei para esta página.
