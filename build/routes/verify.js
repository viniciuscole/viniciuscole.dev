...
;(async () => {
  ...
  const cartao = principal.locator("[data-rotas-editor-form]")
  const canvas = principal.locator("canvas")
  await canvas.focus()
  await page.keyboard.press("ArrowRight")
  await page.keyboard.press("Enter")
  confere("teclado: seta + Enter abre o cartao da via", await cartao.isVisible())
  await page.keyboard.press("Escape")
  confere("teclado: Esc fecha o cartao", !(await cartao.isVisible()))
  confere("teclado: o foco volta ao canvas", await canvas.evaluate((el) => document.activeElement === el))
  await principal.locator("[data-rotas-editor-seletor]").selectOption("2-4")
  await principal.locator("[data-rotas-editor-seletor] + button, .rotas-escolha button").first().click()
  confere("seletor de via abre o cartao", await cartao.isVisible())
  confere("cartao mostra a via escolhida", (await cartao.locator("strong").textContent()).includes("2 → 4"))
  await principal.locator("[data-rotas-editor-cancelar]").click()
